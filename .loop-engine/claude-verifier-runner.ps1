...
- Use `ConsumerLifecycle` `bind-primary` before primary-period scenarios and `bind-secondary` for the period-switch path. Do not supply `calc_month`; the controller reads the governed Deployment configuration and injects `PVAM_BOUND_PERIOD`, `PVAM_CALC_MONTH`, and `PVAM_LEDGER_KEY_PREFIX`.
- After the final required ConsumerObserve for the stage, invoke ConsumerLifecycle restore for the same exact allowlisted deployment/container. A stage cannot complete before ConsumerLifecycle restore succeeds.
- ConsumerLifecycle deployment/container/pod prefix must match a version-controlled consumer_lifecycle_targets entry exactly; an empty target list is intentionally fail-closed. Never guess or broaden a target.
