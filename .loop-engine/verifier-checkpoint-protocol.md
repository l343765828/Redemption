...
- All Kafka/Redis/observation/proof helpers resolve that same scheduler Pod and exact NFS repository from policy. Their inner Python process loads Redis/Dask settings from Candidate `Model.Config`; secrets never cross the proxy request/evidence boundary.
