...
- The Consumer is a governed runtime component. `ConsumerLifecycle` requires an exact version-controlled `consumer_lifecycle_targets` match for namespace + Deployment + container + Pod prefix. Empty target configuration is fail-closed. Dispatch resource scope remains an additional Cycle boundary, not the target definition itself.
- Before any bind mutation, the controller validates the exact container, Pod prefix/scope, selected Pods, governed `PVAM_CALC_MONTH`, and baseline replicas. Only the exact allowlisted container receives UAT env through `--containers=<consumer>`.
...
- `ConsumerObserve` turns producer evidence into end-to-end evidence. It checks fixed consumer-group offset semantics, delivery-ledger `DISPATCHED`, three-chain done keys, scenario-specific exception reasons, and period-drain log evidence before the stage may claim the corresponding scenario complete.
