...
- Kafka production, Redis proofs/cleanup, Consumer observation, P99, and recovery helpers are pinned to the same unique scheduler Pod and exact NFS repository. Their Python launcher imports connection settings from Candidate `Model.Config` inside the Pod; credentials are neither command arguments nor controller evidence.
