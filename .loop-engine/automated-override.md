...
- `RedisDbSize` must be recorded for both `before` and `after`; the final count may not exceed the initial count. `RedisDeleteExactKeys` must prove all requested keys are absent after deletion and its request set must cover every controller-observed Kafka delivery identity plus its exact period/user UserStats business record.
...
- `ConsumerLifecycle` is the only governed way to bind/rebind/restore the WORK-PVAM-02 Consumer. R9 models the actual UAT topology: node3 hosts the shared NFS checkout, Dask executes the tests, and `MessageConsumer.PvEventConsumer` is a temporary process inside the unique Running+Ready scheduler Pod.
- Before process mutation, the controller verifies the version-controlled `consumer_runtime_target`, Cycle deployment/Pod scope, exact scheduler container readiness, Pod UID, Candidate SHA in the shared repository, and role-specific period/calc-month policy. It never runs `kubectl set env`, `scale`, or `rollout restart` for ConsumerLifecycle.
- Bind/rebind uses a controller-owned runtime helper and execution-scoped state under `/tmp`; the helper rejects unmanaged duplicate Consumers and never emits or persists Redis credentials. `ConsumerLifecycle restore` stops only the PID proven to belong to this execution and module, leaving the scheduler Deployment unchanged.
- Kafka production, Redis proofs/cleanup, Consumer observation, P99, and recovery helpers are pinned to the same unique scheduler Pod, exact NFS repository, and policy-defined non-secret Dask/Redis endpoints. The Python launcher rejects a Candidate whose `Model.Config` endpoint values differ, then loads only the Redis credential inside the Pod; credentials are neither command arguments nor controller evidence. Dask profiles use the proxy-supplied policy scheduler address.
- Opus must prove `bind-primary`, the required drain/observation path, `bind-secondary`, and final `restore`. Fable must prove its policy-defined binding/observation set and final `restore`. A bind is accepted only when the controller proves the execution-owned process is running with the expected Candidate, period, calc month, and ledger prefix.
- `ConsumerObserve` is required after policy-defined Kafka scenarios. It must prove consumer-group offset expectations, scenario-specific exception reasons and `has_redis_residue` where applicable, delivery-ledger state, and all three idempotency namespaces (`userstats`, `placement`, `elite`) for scenarios that require full three-chain dispatch. The governed `post-pending-order-conflict` scenario must prove `delivery_status=PENDING`, `has_redis_residue=true`, and unchanged UserStats state after the controller seeds one execution-scoped conflicting order-ledger record.
- Runtime repository integrity is a hard gate: the Candidate-bound stage grant exports mandatory `LOOP_CANDIDATE_SHA`; the workflow cross-checks it against `pushed-sha.txt`; `GitAudit`, `GitUpdate`, `ConsumerLifecycle`, and `ConsumerObserve` evidence must match the Candidate. Pod-side Git commands use the exact discovered repository as `safe.directory`, and an empty Candidate SHA can never disable these checks.
...
- `pending-dispatched-recovery`: the controller first completes a real three-chain event, injects the persisted delivery ledger back to `PENDING` while all three stage idempotency markers remain present, replaces the governed temporary Consumer process, replays the same event, requires `DISPATCHED`, and requires UserStats PV to remain byte-for-byte equivalent at the business-field level.
...
All required Kafka scenarios carry controller-owned `business_snapshot_before`; each `ConsumerObserve` produces `business_snapshot_after` and `business_value_proof_ok`. Pre-dispatch negative scenarios must have zero ledger/idempotency effects and unchanged UserStats snapshots. The post-PENDING conflict scenario instead requires its policy-declared PENDING/order-ledger residue while still requiring unchanged UserStats snapshots. Every delivered identity, including identities created by `UatProof`, must receive exact ledger and all-three-idempotency cleanup; each controller-delivered Kafka period/user pair must also receive exact UserStats business-record cleanup before restore/final verdict.
