...
def _validated_start_contract(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    dask_scheduler = _require_text(payload, "dask_scheduler")
    redis_host = _require_text(payload, "redis_host")
    redis_port = int(payload.get("redis_port") or 0)
    redis_db = int(
        payload.get("redis_db") if payload.get("redis_db") is not None else -1
    )
    if any(char in dask_scheduler for char in "\x00\r\n "):
        raise RuntimeContractError("dask_scheduler is invalid")
    if any(char in redis_host for char in "\x00\r\n "):
        raise RuntimeContractError("redis_host is invalid")
    if not 1 <= redis_port <= 65535:
        raise RuntimeContractError("redis_port is invalid")
    if redis_db < 0:
        raise RuntimeContractError("redis_db is invalid")
...
def _build_child_environment(contract: dict[str, Any]) -> dict[str, str]:
    ...
    expected = {
        "dask_scheduler": contract["dask_scheduler"],
        "redis_host": contract["redis_host"],
        "redis_port": contract["redis_port"],
        "redis_db": contract["redis_db"],
    }
    candidate = {
    ...
    if candidate != expected:
        raise RuntimeContractError(
            "runtime Model.Config does not match policy-pinned endpoints"
        )
