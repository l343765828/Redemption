...
from Model.User.ChangeUserInfoMsg import ChangeUserInfoMsg
...
def consume_loop():
    ...
    actor_future = client.get_dataset("userinfo_actor")
    ...
    try:
        while run:
            ...
            adapter = TypeAdapter(list[ChangeUserInfoMsg])
            try:
            ...
            # region 构建图（只在版本升级时建）
            remote_future = actor.run_update_userinfo(int(key), change_list=models, npartitions=1)
