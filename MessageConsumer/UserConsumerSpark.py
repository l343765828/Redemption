...
from Model.Message.Message import (
    ENVELOPE_USER_SCHEMA, KEY_SCHEMA
)
from Until.SparkCommon import json_list_to_array_bytes

delta_path = Model.Config.SPARK_DELTA_USER


# delta_path = "/mnt/delta/tables/tb_user"
...
def process(batch_df, batch_id, spark, kafka_bootstrap):
    ...
    print("=== New batch:", batch_id, "=== for topic dbserver1.test.tb_user")
    ...
    .withColumn("envelope", from_json(col("value_str"), ENVELOPE_USER_SCHEMA))
    .withColumn("after_id", col("envelope.after.id"))
    .withColumn("after_user", col("envelope.after.user"))
    .withColumn("after_parent", col("envelope.after.parent"))
    .withColumn("after_placementId", col("envelope.after.placementId"))
    .withColumn("after_placementLeg", col("envelope.after.placementLeg"))
    .withColumn("after_updatetime", col("envelope.after.updatetime"))
    .withColumn("before_id", col("envelope.before.id"))
    .withColumn("before_user", col("envelope.before.user"))
    .withColumn("before_parent", col("envelope.before.parent"))
    .withColumn("before_placementId", col("envelope.before.placementId"))
    .withColumn("before_placementLeg", col("envelope.before.placementLeg"))
    ...
    .select(
        ...
        col("key").cast("string").alias("key_str"),
        col("value_str"),
        col("after_id"), col("after_user"), col("after_parent"), col("after_placementId"), col("after_placementLeg"),
        col("after_updatetime"),
        col("before_id"), col("before_user"), col("before_parent"), col("before_placementId"),
        col("before_placementLeg"), col("before_updatetime"),
    ...
    normalized = (key_parsed
                  ...
                  .withColumn("user",
                              coalesce(col("after_user"), col("before_user")))
                  .withColumn("parent",
                              coalesce(col("after_parent"), col("before_parent")))
                  .withColumn("placementId",
                              coalesce(col("after_placementId"), col("before_placementId")))
                  .withColumn("placementLeg",
                              coalesce(col("after_placementLeg"), col("before_placementLeg")))
                  ...
                  .select("topic", "partition", "offset", "kafka_ts", "key_str",
                          "value_str", "id", "user", "parent", "placementId", "placementLeg", "updatetime", "op",
                          "ts_ms", "event_ts", "is_tombstone")
                  )
    ...
    latest_per_id = best_per_id.select(
        col("id"),
        col("best.user").alias("user"),
        col("best.parent").alias("parent"),
        col("best.placementId").alias("placementId"),
        col("best.placementLeg").alias("placementLeg"),
    ...
    # 允许 Delta Lake 自动合并新字段 (Schema Evolution)
