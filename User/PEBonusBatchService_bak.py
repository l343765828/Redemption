...
class PEBonusBatchServiceGPU:
    ...
    def _dedup_or_raise(ddf: dask_cudf.DataFrame, value_cols: List[str], key_cols: List[str],
        ...
        sizes = ddf.groupby(key_cols).size()
        has_duplicates = (sizes > 1).any().compute()
        if has_duplicates:
            bad_keys = sizes[sizes > 1].head(10).index.to_pandas().tolist()
    ...
    def _apply_nested_push_up_net(self, snapshot: dask_cudf.DataFrame, client) -> dask_cudf.DataFrame:
        ...
        while iteration < 5000:
            ...
            # 【修复点】将 READY_PID 转为关联键 user_id，并引入专用的标记列 MARK_FLAG 避免后续 drop 误伤
    ...
    def _reconstruct_elite_snapshot(self,
        ...
        ddf_tree['parent_id'] = ddf_tree['parent_id'].replace('0', 0).replace(0, 0).fillna(0).astype('int64')
    ...
    def execute_batch(self,
        ...
        mid1 = mid1.merge(ddf_user[['id', 'country_id', 'user_name', 'real_name']], left_on='USER_ID', right_on='id',
                          how='left')
        mid1 = mid1.drop(columns=['id'])
        ...
        if len_a > 0:
            ...
            detail_a = detail_a.merge(ddf_user[['id', 'user_name', 'real_name']], left_on='USER_ID', right_on='id',
                                      how='left')
            detail_a = detail_a.drop(columns=['id'])
            detail_a = detail_a.merge(ddf_user[['id', 'user_name', 'real_name']], left_on='SOURCE_USER_ID',
                                      right_on='id', how='left', suffixes=('_BONUS', '_SOURCE'))
            detail_a = detail_a.drop(columns=['id'])
        ...
        if len_b > 0:
            detail_b = detail_b.merge(ddf_user[['id', 'user_name', 'real_name']], left_on='USER_ID', right_on='id',
                                      how='left')
            detail_b = detail_b.drop(columns=['id'])
