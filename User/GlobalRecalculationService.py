...
class GlobalRecalculationService(UserStatsService):
    ...
    @classmethod
    def _recalc_gpv_real_unreal(cls, node: UserStats) -> None:
        node.gpv_real, node.gpv_unreal = cls._calc_gpv_real_unreal(node.gpv or 0)
    ...
    def _extract_comparable_state(node: UserStats) -> Dict[str, Any]:
        ...
        return {
            "gpv": node.gpv or 0,
            "gpv_real": node.gpv_real or 0,
            "gpv_unreal": node.gpv_unreal or 0,
    ...
    def _process_parent_batch(
        ...
        for pid in parent_ids:
            ...
            # 兼容未在 UserStatsService._recalc_rank 内同步 real/unreal 的旧版本：
            # 全量重算在最终计算 rank 后强制刷新一次，保证落库口径完整。
            self._recalc_gpv_real_unreal(p_node)
    ...
    def _is_zero_user_stats(node: UserStats) -> bool:
        ...
        if node.gpv_real not in (None, 0): return False
        if node.gpv_unreal not in (None, 0): return False
    ...
    try:
        ...
        for uid in ["1", "2", "3", "9", "10", "13"]:
            try:
                ...
                print(f"User {uid}: GPV={stats.gpv}, Real={stats.gpv_real}, Unreal={stats.gpv_unreal}, "
                      f"Rank={stats.rank}, HighestRank={honor.highest_rank}, Elite={stats.is_elite}, "
