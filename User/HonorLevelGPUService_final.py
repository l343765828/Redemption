...
class HonorLevelGPUService:
    ...
    def _empty_old_honor_snapshot(self, user_id_dtype="int64") -> cudf.DataFrame:
        return cudf.DataFrame(
            {
                "user_id": cudf.Series(dtype=user_id_dtype),
                "old_honor_calc_id": cudf.Series(dtype="int32"),
                "old_honor_lv": cudf.Series(dtype="object"),
            }
        )

    def build_honor_snapshot(self, df_result: cudf.DataFrame) -> cudf.DataFrame:
        """
        将本次结算结果整理成可持久化的旧状态快照，供下次 Diff 使用。
        """
        if df_result is None or len(df_result) == 0:
            return self._empty_old_honor_snapshot()

        required = {"user_id", "last_honor_calc_id"}
        missing = required - set(df_result.columns)
        if missing:
            raise ValueError(f"df_result 缺少必需列: {sorted(missing)}")

        snapshot = (
            df_result[["user_id", "last_honor_calc_id"]]
            .rename(columns={"last_honor_calc_id": "old_honor_calc_id"})
            .copy()
            .reset_index(drop=True)
        )
        snapshot["old_honor_calc_id"] = snapshot["old_honor_calc_id"].fillna(0).astype("int32")

        if bool(snapshot["user_id"].duplicated().any()):
            raise ValueError("df_result.user_id 存在重复，无法构建快照。")

        if "last_honor_lv" in df_result.columns:
            snapshot["old_honor_lv"] = df_result["last_honor_lv"].reset_index(drop=True)
        else:
            snapshot["old_honor_lv"] = self._new_object_series(len(snapshot), index=snapshot.index)

        return snapshot[["user_id", "old_honor_calc_id", "old_honor_lv"]].reset_index(drop=True)

    def extract_changed_events(
        self,
        ...
        约定：
        - df_new 来源于 recompute_all_gpu 的返回结果；
        - df_old 来源于“上一次已同步到 elken 的状态快照”；
        - 返回值为 Pandas DataFrame，便于序列化后推送 Kafka。
        """
        ...
        required_new = {"user_id", "last_honor_calc_id"}
        ...
        if bool(df_new["user_id"].astype(user_id_dtype).duplicated().any()):
            raise ValueError("df_new.user_id 存在重复，无法进行状态 Diff。")

        new_subset = df_new[["user_id", "last_honor_calc_id"]].copy().reset_index(drop=True)
        new_subset["user_id"] = new_subset["user_id"].astype(user_id_dtype)
        new_subset["last_honor_calc_id"] = new_subset["last_honor_calc_id"].fillna(0).astype("int32")
        if "last_honor_lv" in df_new.columns:
            new_subset["last_honor_lv"] = df_new["last_honor_lv"].reset_index(drop=True)
        else:
            new_subset["last_honor_lv"] = self._new_object_series(len(new_subset), index=new_subset.index)

        if df_old is None or len(df_old) == 0:
            old_subset = self._empty_old_honor_snapshot(user_id_dtype)
        else:
            required_old = {"user_id", "old_honor_calc_id"}
            ...
            if bool(old_subset["user_id"].duplicated().any()):
                raise ValueError("df_old.user_id 存在重复，无法进行状态 Diff。")
            old_subset["old_honor_calc_id"] = old_subset["old_honor_calc_id"].fillna(0).astype("int32")
            if "old_honor_lv" not in old_subset.columns:
                old_subset["old_honor_lv"] = self._new_object_series(len(old_subset), index=old_subset.index)
            else:
                old_subset["old_honor_lv"] = old_subset["old_honor_lv"].reset_index(drop=True)
            old_subset = old_subset[["user_id", "old_honor_calc_id", "old_honor_lv"]].copy().reset_index(drop=True)

        df_diff = new_subset.merge(old_subset, on="user_id", how="left")
        df_diff["old_honor_calc_id"] = df_diff["old_honor_calc_id"].fillna(0).astype("int32")
        if "old_honor_lv" not in df_diff.columns:
            df_diff["old_honor_lv"] = self._new_object_series(len(df_diff), index=df_diff.index)

        mask_changed = df_diff["last_honor_calc_id"] != df_diff["old_honor_calc_id"]
        df_changed = df_diff[mask_changed].copy().reset_index(drop=True)

        pdf_changed = df_changed.to_pandas()
        if len(pdf_changed) == 0:
            elapsed = time.perf_counter() - t0
            LOG.info("Honor Diff 完成，耗时 %.4fs，无状态变化。", elapsed)
            return pd.DataFrame(
                columns=[
                    "event_id",
                    "event_type",
        ...
        pdf_changed = pdf_changed.rename(
            columns={
                "last_honor_calc_id": "new_honor_calc_id",
                "last_honor_lv": "new_honor_lv",
            }
        )
        pdf_changed["event_type"] = "HONOR_LEVEL_CHANGED"
        pdf_changed["period_num"] = int(period_num)
        pdf_changed["calc_month"] = int(calc_month)
        pdf_changed["source_order_id"] = source_order_id
        pdf_changed["changed_direction"] = pdf_changed.apply(
            lambda row: "UP" if row["new_honor_calc_id"] > row["old_honor_calc_id"] else "DOWN",
            axis=1,
        )
        pdf_changed["event_ts"] = int(time.time())
        pdf_changed["event_id"] = pdf_changed.apply(
            lambda row: (
                f"honor:last:{period_num}:{calc_month}:{row['user_id']}:"
                f"{row['new_honor_calc_id']}:{source_order_id}"
            ),
            axis=1,
        )
        ...
        elapsed = time.perf_counter() - t0
        LOG.info("Honor Diff 完成，耗时 %.4fs，变化记录 %d 条。", elapsed, len(pdf_changed))
        return pdf_changed

    def publish_changed_events(
        self,
        events_pdf: pd.DataFrame,
        producer,
        topic: str = "redemption.honor.changed",
        flush: bool = True,
    ) -> int:
        """
        将 extract_changed_events 的结果推送到 Kafka。

        兼容两类 producer：
        - confluent_kafka.Producer: 使用 produce(topic=..., key=..., value=...)
        - kafka.KafkaProducer: 使用 send(topic, key=..., value=...)
        """
        ...
        LOG.info("Honor 变化事件已推送到 Kafka，topic=%s, 条数=%d", topic, sent)
        return sent
    ...
    def run_honor_job(
        ...
        """
        执行当期 Honor 计算，并在需要时推送增量事件。

        返回：
        - df_result: 本次全量结算结果
        - df_layers: 截层明细
        - events_pdf: 本次需要推送的变化事件
        - new_snapshot: 供调用方持久化、作为下次 Diff 基线的最新快照
        """
        df_result, df_layers = self.recompute_all_gpu(df_honor_levels=df_honor_levels)
        events_pdf = self.extract_changed_events(
            df_new=df_result,
        ...
        if producer is not None and len(events_pdf) > 0:
            self.publish_changed_events(events_pdf=events_pdf, producer=producer, topic=topic, flush=flush)
        new_snapshot = self.build_honor_snapshot(df_result)
        return df_result, df_layers, events_pdf, new_snapshot
    ...
    def recompute_all_gpu(
        ...
        # 过滤出“gpv_unreal>0且parent_se_id!=0”的节点，并按parent_se_id!=0分组对gpv_unreal求和
        ...
        # target_se = parent_uid==parent_se_id?grandpa_se_id:parent_se_id，target_se!=0分组对gpv_real求和
        ...
        # region agg_u联查agg_r，合并LB_PV LB_PV=lb_unreal+lb_real -> df_lb
        ...
        # region 找出活跃SE的id list -> active_set
        ...
        # region 获取 每个SE的最大长度 和 每层长度SE的业绩 （奖金计算）
        LOG.info("深度递归 & 逐层业绩...")
        ...
        df_f = df_base[
            ["user_id", "parent_uid", "parent_se_id", "last_elite_calc_id", "limit_honor_calc_id", "is_active"]
        ].copy().reset_index(drop=True)
        ...
        # region ori_honor_calc_id：算出honor level的能力等级
        ...
        # region bonus_honor_calc_id：级别等级 = min(能力等级, 会员资格等级)
    ...
    def _compute_width(
        ...
        # region 分组parent_se、branch_key，获取se_num（深度）最大的max
